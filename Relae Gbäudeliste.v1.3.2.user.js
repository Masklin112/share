...
(async function() {
    ...
    // --- 3. Code-Block NUR für das LISTENFENSTER (Tab B) ---
        ...
        let regionFilter = 'all'; // Dies ist der Kreis-Filter
        let wachenFilter = -1;
        ...
        function createBuildingMenu() {
            ...
            // MODIFIZIERT: HTML um Suchfeld erweitert
            buildingMenuList.innerHTML = `
                ...
                <div class="filter-section" id="region-filters" style="display: none;"> <select id="region-filter-select">
                        <option value="all">Alle Regionen / Kreise</option>
                     </select>
                </div>
        ...
        // MODIFIZIERT: Liest jetzt auch das Suchfeld aus
        ...
        // MODIFIZIERT: Befüllt nur noch die Region/Kreis-Filter
        function populateRegionFilter() {
            ...
            const uniqueRegions = [...new Set(wachen.map(w => w.region))].filter(Boolean).sort();
            ...
            if (uniqueRegions.length > 0) {
                ...
                container.style.display = 'flex'; // Anzeigen
            } else {
                container.style.display = 'none'; // Verstecken, wenn keine Regionen da
            }
        }

        // Befüllt die Gebäudetyp-Filter-Buttons
        ...
        async function setupListWindow() {
            ...
            // Event-Listener für Wachen-Typ-Buttons
            ...
            // Event-Listener für Region/Kreis
            document.addEventListener('change', (event) => {
                if (event.target.matches('#region-filter-select')) {
                     regionFilter = event.target.value;
                     updateBuildingMenuListList(); // Hauptliste aktualisieren
                }
            });
