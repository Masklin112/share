// ==UserScript==
// @name          Personalzuweiser auf Koks (Fixed v5.2.7 - Loop Double-Trigger Fix)
// @namespace     personalzuweiser.leitstellenspiel.de
// @version       5.2.7
// @license       BSD-3-Clause
// @author        BOS-Ernie, Masklin, BAHendrik (modifiziert durch KI)
// @description   Kombiniert die verbesserte Zuweisungslogik von 4.5.0 mit den Automations-Features von 4.3.1. Fix: Verhindert doppelte Ausführung beim Seitenwechsel im Loop.
...
(async function () {
    ...
    // NEU: Key für den Status 6 Schalter
    const setPreviousToS6Key = localStoragePrefix + "setPreviousToS6";
    ...
    const vehiclesConfigurationOverride = [
        ...
        // FIX: Rettungshunde THW mit korrektem Key "thw_rescue_dogs"
    ...
    async function initVehiclesConfiguration() {
        ...
        if (cached) {
            try {
                ...
                if (parsed.trainings && Object.keys(parsed.trainings).length > 0 && Array.isArray(parsed.data) && parsed.data.length > 0 && parsed.lastUpdate > Date.now() - storageTtl) {
                    ...
                    // FIX v5.1.4: Hard-Overrides auch auf Cache anwenden
                    vehiclesConfiguration = applyVehicleConfigurationOverride(vehiclesConfiguration);

                    // FIX v5.1.5: ZWINGENDES Überschreiben des Labels, damit es in der Liste auftaucht.
                    allTrainings['thw_rescue_dogs'] = 'Rettungshundeführer (THW)';
        ...
        try {
            ...
            // FIX v5.1.5: ZWINGENDES Hinzufügen/Überschreiben
            allTrainings['thw_rescue_dogs'] = 'Rettungshundeführer (THW)';
    ...
    function applyUserConfiguration() {
        ...
        if (userConfigStr) {
            ...
            vehiclesConfiguration = vehiclesConfiguration.map(defaultConfig => {
                ...
                // FIX v5.1.5: "Selbstheilung" für MTW-O (ID 93).
                if (defaultConfig.id === 93 && userOverride && (!userOverride.training || userOverride.training.length === 0)) {
                    log("🔧 Repariere fehlerhafte User-Config für MTW-O (THW Hunde fehlen).");
    ...
    // Die runAssignmentProcess Logik (v5.3.1 - Button-Unabhängige Erkennung)
    // Die runAssignmentProcess Logik (v5.4.0 - Transit-Schutz: DGL bleibt S6)
    async function runAssignmentProcess(vehicleIds, unassignMode = false, isSubProcess = false, uiContainer = null, skipGridCreation = false) {
        return new Promise(async (resolve, reject) => {
            ...
            const setPreviousToS6 = JSON.parse(localStorage.getItem(setPreviousToS6Key)) || false;
            const currentVehicleIdsStr = vehicleIds.map(String);

            let incompleteVehicles = new Set();
            let vehiclesToSetToS6 = new Set();
            ...
            try {
                ...
                if (!skipGridCreation && vehicleDetails.length > 0) {
                    createVehicleGrid(vehicleDetails, uiContainer);
                }
                ...
                if (unassignMode) {
                    ...
                    if(sourceVehicleForPersonnel) {
                        try {
                            ...
                            // --- TEIL A: BEREITS GEBUNDENES PERSONAL PRÜFEN ---
                            // Wenn wir nur den DGL bearbeiten, ist Mila vielleicht schon zugewiesen, aber physikalisch weg.
                            // Wir müssen das Fahrzeug (DGL) also auf S6 zwingen.
                            if (setPreviousToS6) {
                                const personalRows = dom.querySelectorAll("tr[data-filterable-by]");
                                personalRows.forEach((row) => {
                                    // 1. Wohin gebunden? (Letzte Spalte)
                                    let assignedToVehicleId = null;
                                    const actionCol = row.children[row.children.length - 1];
                                    if (actionCol) {
                                        const bindingLink = Array.from(actionCol.querySelectorAll('a')).find(a => a.href.includes('/vehicles/') && !a.classList.contains('btn'));
                                        if (bindingLink) assignedToVehicleId = bindingLink.href.match(/\/vehicles\/(\d+)/)?.[1];
                                    }

                                    // 2. Wo sitzt sie? (Spalte 3)
                                    const statusCol = row.children[2];
                                    const currentVehicleLink = statusCol ? statusCol.querySelector('a[href*="/vehicles/"]') : null;
                                    const sittingInVehicleId = currentVehicleLink ? currentVehicleLink.href.match(/\/vehicles\/(\d+)/)?.[1] : null;

                                    if (assignedToVehicleId && sittingInVehicleId && assignedToVehicleId !== sittingInVehicleId) {
                                        // Fall 1: Das "Opfer" (wo sie sitzt) ist nicht Teil des Prozesses -> Opfer auf S6
                                        if (!currentVehicleIdsStr.includes(sittingInVehicleId)) {
                                            vehiclesToSetToS6.add(sittingInVehicleId);
                                        }
                                        // Fall 2: Das "Ziel" (DGL) ist Teil des Prozesses -> Ziel auf S6 zwingen!
                                        if (currentVehicleIdsStr.includes(assignedToVehicleId)) {
                                            console.log(`[PZ-Mod] 🛑 Zwinge S6 für Ziel-Fahrzeug ${assignedToVehicleId} (Personal ist noch in ${sittingInVehicleId})`);
                                            incompleteVehicles.add(Number(assignedToVehicleId));
                                        }
                                    }
                                });
                            }

                            // --- TEIL B: NEU-ZUWEISUNG (Pool aufbauen) ---
                            availablePersonnel = Array.from(dom.querySelectorAll("tr[data-filterable-by]")).map(row => {
                                ...
                                const statusCol = row.children[2];
                                const currentVehicleLink = statusCol ? statusCol.querySelector('a[href*="/vehicles/"]') : null;
                                const currentVehicleId = currentVehicleLink ? currentVehicleLink.href.match(/\/vehicles\/(\d+)/)?.[1] : null;

                                if (!assignButton && !currentVehicleId && row.children[2].innerText.trim().startsWith("Im Unterricht")) return null;
                                const idButton = row.querySelector("a[personal_id]");
                                if (!idButton) return null;

                                return {
                                    id: idButton.getAttribute("personal_id"),
                        ...
                        for (const vehicle of vehicleDetails) {
                            ...
                            for (const req of (config.training || [])) {
                                for (let i = 0; i < req.number && assignedPersonnelForThisVehicle.length < config.maxStaff; i++) {
                                    ...
                                    if (personIndex > -1) {
                                        const person = personnelPool.splice(personIndex, 1)[0];
                                        assignedPersonnelForThisVehicle.push(person);

                                        // Wenn wir jemanden zuweisen, der woanders sitzt
                                        if (setPreviousToS6 && person.currentVehicleId && person.currentVehicleId !== String(vehicle.id)) {
                                            // Opfer auf S6
                                            if (!currentVehicleIdsStr.includes(person.currentVehicleId)) {
                                                vehiclesToSetToS6.add(person.currentVehicleId);
                                            }
                                            // ZIEL (DGL) AUCH AUF S6 ZWINGEN!
                                            // Weil das Personal ja noch im Transit ist
                                            console.log(`[PZ-Mod] 🛑 Zwinge S6 für ${vehicle.id} (Personal kommt aus ${person.currentVehicleId})`);
                            ...
                            if ((config.training || []).length === 0 && assignedPersonnelForThisVehicle.length < config.maxStaff) {
                                ...
                                for(let k=0; k<needed; k++) {
                                    const pIndex = personnelPool.findIndex(p => p.qualifications.length === 0);
                                    if(pIndex > -1) {
                                        const person = personnelPool.splice(pIndex, 1)[0];
                                        assignedPersonnelForThisVehicle.push(person);
                                        ...
                                        if (setPreviousToS6 && person.currentVehicleId && person.currentVehicleId !== String(vehicle.id)) {
                                            ...
                                            console.log(`[PZ-Mod] 🛑 Zwinge S6 für ${vehicle.id} (Auffüll-Personal aus ${person.currentVehicleId})`);
                                            incompleteVehicles.add(vehicle.id);
                                        }
                                    }
                                }
                            ...
                            if (assignedPersonnelForThisVehicle.length < totalRequestedPersonnel) { incompleteVehicles.add(vehicle.id); }
                        }
                        if (assignTasks.length > 0) { await taskManager(assignTasks); log("✅ Zuweisungen gesendet."); }
                    }
                }

                // PHASE 4: Status setzen
                log("📊 Phase 4: Setze FMS-Status...");
                const fmsTasks = [];

                if (vehiclesToSetToS6.size > 0) {
                    log(`⚠️ Setze ${vehiclesToSetToS6.size} beraubte Fahrzeuge auf Status 6...`);
                    vehiclesToSetToS6.forEach(vid => {
                         fmsTasks.push({ method: 'GET', url: `/vehicles/${vid}/set_fms/6`, vehicleId: vid });
                    });
                }

                vehicleDetails.forEach(v => {
                    const vehicleId = String(v.id);
                    // WICHTIG: Hier greift jetzt die incompleteVehicles Logik, die den DGL schützt
                ...
                if (!isSubProcess) { log("🏁 Prozess abgeschlossen!"); setTimeout(() => window.location.reload(), 5000); }
    ...
    function addVehicleAssignmentPageButtons() {
        const assignHotkey="s",resetHotkey="x",nextVehicleHotkey="d",previousVehicleHotkey="a";
        const container = document.querySelector(".vehicles-education-filter-box");
        if (!container) return;

        const group = document.createElement("div");
        group.className = "btn-group", group.style.marginLeft = "10px";

        const assignBtn = document.createElement("button");
        assignBtn.className = "btn btn-success";
        assignBtn.innerHTML = '<span class="glyphicon glyphicon-ok"></span> Zuweisen';
        assignBtn.title = `Optimal zuweisen (${assignHotkey.toUpperCase()})`;
        // FIX: Click-Handler für Einzelzuweisung
        assignBtn.addEventListener("click", () => {
            if (currentVehicleId) {
                runAssignmentProcess([currentVehicleId], false);
            } else {
                alert("Fahrzeug-ID konnte nicht ermittelt werden.");
            }
        });

        const resetBtn = document.createElement("button");
        resetBtn.className = "btn btn-danger";
        resetBtn.innerHTML = '<span class="glyphicon glyphicon-trash"></span> Leeren';
        resetBtn.title = `Personal entfernen (${resetHotkey.toUpperCase()})`;
        resetBtn.addEventListener("click", resetVehicle);

        group.append(assignBtn, resetBtn), container.appendChild(group);
        ...
        document.addEventListener("keydown", async e => {
            ...
            if (key === assignHotkey) {
                e.preventDefault();
                assignBtn.click();
            } else if (key === resetHotkey) {
                e.preventDefault();
                resetVehicle();
            } else if (key === nextVehicleHotkey) {
                e.preventDefault();
                document.querySelectorAll(".btn-group.pull-right a")[1]?.click();
            } else if (key === previousVehicleHotkey) {
                e.preventDefault();
    ...
    // FIX v5.2.7: Gibt jetzt true zurück, wenn Arbeit verrichtet wurde, um Doppelstarts zu vermeiden
    ...
    function addBuildingPageUI() {
        ...
        let targetElement = document.querySelector("#vehicle.tab-pane") || document.querySelector(".tab-content") || document.getElementById('vehicle_table');
        ...
        // NEU: Toggle für Status 6 Logik
        const s6Container = document.createElement('div');
        s6Container.className = 'lss-toggle-switch-container';
        s6Container.title = 'Wenn Personal von einem Fahrzeug abgezogen wird, dieses Fahrzeug auf Status 6 setzen.';
        s6Container.style.color = '#cbd5e0';
        const s6Label = document.createElement('label');
        s6Label.className = 'lss-toggle-switch';
        const s6Input = document.createElement('input');
        s6Input.type = 'checkbox';
        s6Input.id = 'pz-s6-toggle';
        s6Input.checked = JSON.parse(localStorage.getItem(setPreviousToS6Key)) || false;
        const s6Slider = document.createElement('span');
        s6Slider.className = 'lss-slider';
        s6Label.append(s6Input, s6Slider);
        s6Container.append(document.createTextNode('Alt-Fzg S6'), s6Label);
        s6Input.addEventListener('change', () => {
            localStorage.setItem(setPreviousToS6Key, s6Input.checked);
            log(`Status 6 Modus geändert auf: ${s6Input.checked ? 'Aktiviert' : 'Deaktiviert'}`);
        });
        addonContainer.appendChild(s6Container);
    ...
    async function main() {
        ...
        if (isVehicleAssignmentPage) {
            ...
            if (isBuildingOverviewPage || isBuildingExpansionPage) {
                // FIX v5.2.7: Prüfung, ob der Controller Arbeit verrichtet hat (return value)
                const controllerDidWork = await handleSequenceController();

                // Wenn der Controller NICHTS getan hat (also kein laufender Prozess beendet wurde),
                // UND der Loop aktiv ist, DANN dürfen wir eine neue Sequenz starten.
                const superLoopState = getState(superLoopStateKey);
                if (!controllerDidWork && superLoopState?.active && !getState(sequenceStateKey)) {
            ...
            const mainUiObserver = new MutationObserver((_, obs) => {
                // Wir suchen jetzt nach dem "Fahrzeuge"-Tab Link
                const vehicleTab = document.querySelector('a[href="#vehicle"]');
                if (vehicleTab) {
                    log("👀 Haupt-UI Observer: Fahrzeuge-Tab gefunden. Initialisiere UI...");
                    addBuildingPageUI();

                    // Analyse nur starten, wenn tatsächlich Fahrzeuge da sind
                    if (document.getElementById('vehicle_table')) {
